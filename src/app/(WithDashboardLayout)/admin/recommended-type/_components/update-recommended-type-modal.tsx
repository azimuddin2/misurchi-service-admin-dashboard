...
import { useUpdateRecommendedTypeMutation } from '@/redux/features/recommendedType/recommendedTypeApi';
import { AppButton } from '@/components/shared/app-button';
import { ArrowRight } from 'lucide-react';
import { useEffect } from 'react';
import { TRecommendedType } from '@/types/recommended.type';

interface UpdateRecommendedTypeModalProps {
  isOpen: boolean;
  onOpenChange: (isOpen: boolean) => void;
  refetch?: () => void;
  recommendedTypeData: TRecommendedType | null;
}
...
const UpdateRecommendedTypeModal = ({
  isOpen,
  onOpenChange,
  refetch,
  recommendedTypeData,
}: UpdateRecommendedTypeModalProps) => {
  const [updateRecommendedType] = useUpdateRecommendedTypeMutation();
  ...
  useEffect(() => {
    if (recommendedTypeData) {
      form.reset({ name: recommendedTypeData.name });
    }
  }, [recommendedTypeData, form]);

  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    if (!recommendedTypeData?._id) return;

    const toastId = toast.loading('Updating recommended type...');
    try {
      const res = await updateRecommendedType({
        id: recommendedTypeData._id,
        data,
      }).unwrap();
      toast.success(res.message || 'Recommended type updated successfully');
      form.reset();
      onOpenChange(false);
      refetch?.();
    } catch (error: any) {
      toast.error(error?.data?.message || 'Failed to update recommended type');
    } finally {
        ...
        <Form {...form}>
          ...
          >
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Recommended Type Name</FormLabel>
                  <FormControl>
                    <Input
                      required
...
export default UpdateRecommendedTypeModal;
