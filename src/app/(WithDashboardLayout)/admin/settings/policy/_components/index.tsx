...
export default function PolicyForm() {
  ...
  useEffect(() => {
    if (policyData?.data?.content) {
      setValue('content', policyData.data.content);
    }
  }, [policyData, setValue]);
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    try {
      const res = await AddPolicy(data).unwrap();
      toast.success(res.message || 'Policy saved successfully');
  ...
  return (
    <div className="rounded max-w-7xl">
      <h2 className="text-xl font-medium mb-3 ml-1">Policy</h2>
      <Form {...form}>
